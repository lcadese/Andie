...
public abstract class ImageAction extends AbstractAction {
     ...
     */
    ImageAction(String name, ImageIcon icon, String desc, Integer key, Integer modifier){
        super(name, icon);
        if (desc != null) {
           putValue(SHORT_DESCRIPTION, desc);
        }
        if (key != null && modifier != null) {
            putValue(ACCELERATOR_KEY, KeyStroke.getKeyStroke(key.intValue(), modifier.intValue()));
        }
    }
